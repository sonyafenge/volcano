...
start_ts_vcjob=$8  # 第三个时间段的开始时间
end_ts_vcjob=$9    # 第三个时间段的结束时间
aff=$6
...
# 校验时间参数个数，必须为7个
if [ $# -ne 9 ]; then
  echo "must have group,start_ts_default,end_ts_default,start_ts_volcano,end_ts_volcano,aff,use_mini,start_ts_vcjob,end_ts_vcjob"
  exit 1
fi
...
QUERY_VCJOB="count(kube_pod_status_phase\{phase=%22Succeeded%22,namespace=%22default%22\})"
...
echo "$PROMETHEUS_URL?query=$QUERY_VCJOB&start=$start_ts_vcjob&end=$end_ts_vcjob&step=$STEP"
...
# 获取第三个时间段的数据
DATA3=$(curl -s "$PROMETHEUS_URL?query=$QUERY_VCJOB&start=$start_ts_vcjob&end=$end_ts_vcjob&step=$STEP" | jq '.data.result[0].values')
...
start_ts_vcjob_ALIGNED=$start_ts_default  # 将第三个时间段的起点对齐到第一个时间段

# 处理数据，将数据保存到文件
echo "# Time (aligned)  Count1  Count2 Count3" > data.txt
for (( i=0; i<$(echo $DATA1 | jq 'length'); i++ )); do
  ...
  TIME3=$(echo $DATA3 | jq -r ".[$i][0]")
  COUNT3=$(echo $DATA3 | jq -r ".[$i][1]")

  # 对齐时间到 start_ts_default
  ALIGNED_TIME1=$((TIME1 - start_ts_default_ALIGNED))
  ALIGNED_TIME2=$((TIME2 - start_ts_volcano_ALIGNED))
  ALIGNED_TIME3=$((TIME3 - start_ts_vcjob_ALIGNED))
  # 将对齐后的时间和 count 值写入文件
  echo "$ALIGNED_TIME1  $COUNT1  $COUNT2 $COUNT3" >> data.txt
done
...
set output '../img/kamaji/$img_name.png'
...
plot "data.txt" using 1:2 with lines title "Default", \
     ...
     "data.txt" using 1:4 with lines title "Vcjob"
EOF
